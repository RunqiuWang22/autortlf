...
generate_baseline_table <- function(dataset, meta, global_config, verbose = TRUE) {
  ...
  for (var_info in meta$variables) {
    ...
    if (var_info$type == "continuous") {
      var_results <- process_continuous_variable(
        dataset, var_info, treatment_var, col_structure,
        treatment_levels, display_options, decimals
      )
    } else if (var_info$type == "categorical") {
      var_results <- process_categorical_variable(
        dataset, var_info, treatment_var, col_structure,
        treatment_levels, display_options, decimals
      )
    } else {
  ...
  # Combine all results
  raw_table <- do.call(rbind, all_results)
  rownames(raw_table) <- NULL

  # Inject group header rows for pageby_row = "first_row"
  group_names <- unique(raw_table$group)
  header_list <- list()
  for (g in group_names) {
    header_row <- raw_table[1, ]
    header_row[1, ] <- ""
    header_row$group <- g
    header_row$Variable <- paste0("\\b ", g) # Add formatting directly or let r2rtf handle it (r2rtf will pick it up)
    sub_table <- raw_table[raw_table$group == g, ]
    sub_table$Variable <- paste0("  ", sub_table$Variable) # indent children
    header_list[[g]] <- rbind(header_row, sub_table)
  }
  final_table <- do.call(rbind, header_list)
...
generate_baseline_rtf <- function(baseline_table, meta, global_config, verbose = TRUE) {
  ...
  rtf_obj <- baseline_table[-1, ] %>%
    ...
    rtf_title(
      meta$title %||% "Baseline Characteristics",
      meta$subtitle %||% ""
    ) %>%
    rtf_colheader(
