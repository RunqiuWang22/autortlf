# =============================================================================
# AE Specific Analysis Runner
# =============================================================================
# Program: run_ae0specific.R
# Purpose: Simple runner for AE specific analysis
# Version: 1.0.0
# Created: 2025-10-16
...
# Source the complete AE analysis function
source("function/standard/ae0specific.R")

#' Main function for AE analysis
#'
#' @param metadata_file Path to metadata YAML file (default: pganalysis/metadata/ae0specific.yaml)
#' @param global_config_file Path to global config file (default: pgconfig/metadata/study_config.yaml)
#' @param verbose Whether to print progress messages
main <- function(metadata_file = "pganalysis/metadata/ae0specific.yaml",
  ...
  if (!file.exists(global_config_file)) {
   ...
   if (verbose) cat("=== Starting AE Analysis Pipeline ===\n")
  ...
  if (verbose) {
    ...
    cat("Analysis for:", meta$title %||% "AE Table", "\n")
  }
  ...
  tryCatch({
    ...
    if (verbose) {
      ...
      cat("Table rows:", nrow(results$ae_table), "\n")
      cat("Table columns:", ncol(results$ae_table), "\n")
    ...
    end_time <- Sys.time()
    duration_seconds <- difftime(end_time, start_time, units = "secs")
    finalize_log_with_messages(log_file, success = FALSE, input_files = list(metadata_file = metadata_file, global_config_file = global_config_file), output_files = list(log_file = log_file), duration_seconds = duration_seconds)
...
if (!interactive()) {
  ...
  if (length(args) >= 1) {
    ...
    metadata_file <- "pganalysis/metadata/ae0specific.yaml"
  }
